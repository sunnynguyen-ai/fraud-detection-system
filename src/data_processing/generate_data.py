...
def create_fraud_dataset(n_samples=100000):
    ...
    df.loc[normal_mask, 'Amount'] = np.random.lognormal(mean=3, sigma=1, size=normal_mask.sum())
    ...
    fraud_amounts.extend(np.random.lognormal(mean=2.5, sigma=0.8, size=int(0.7 * n_fraud)))
    # 20% medium fraud amounts
    fraud_amounts.extend(np.random.lognormal(mean=4, sigma=0.5, size=int(0.2 * n_fraud)))
    # 10% large fraud amounts
    fraud_amounts.extend(np.random.lognormal(mean=6, sigma=0.3, size=n_fraud - len(fraud_amounts)))
...
def generate_and_save_data():
    ...
    print(f"Fraud rate: {fraud_data['Class'].mean():.4f} ({fraud_data['Class'].mean()*100:.2f}%)")
