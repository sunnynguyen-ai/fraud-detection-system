...
class FeatureStore:
    ...
    def store_features(self, feature_name: str, features: pd.DataFrame, 
...
class AdvancedFeatureEngineering:
    ...
    def create_amount_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "Amount" in df.columns:
            ...
            df["amount_second_digit"] = df["Amount"].apply(
                lambda x: int(str(x).replace(".", "")[1]) if len(str(x).replace(".", "")) > 1 else 0
            )
        ...
        if "Time" in df.columns and "Amount" in df.columns:
            ...
            for window in [10, 30, 100]:
                df_sorted[f"velocity_{window}"] = (
                    ...
                    (df_sorted["time_since_last_fillna"].rolling(window, min_periods=1).sum() + 1e-8)
                )
            ...
            # Frequency features
            df_sorted["tx_frequency_hour"] = 1 / (df_sorted["time_since_last_fillna"] / 3600 + 1e-8)
    ...
    def create_statistical_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if v_columns:
            ...
            df["v_mad"] = np.abs(v_data - v_data.median(axis=1).values.reshape(-1, 1)).median(axis=1)
            ...
            # Higher moments
            df["v_moment_3"] = ((v_data - v_data.mean(axis=1).values.reshape(-1, 1)) ** 3).mean(axis=1)
            df["v_moment_4"] = ((v_data - v_data.mean(axis=1).values.reshape(-1, 1)) ** 4).mean(axis=1)
            ...
            if column != self.target_column:
                ...
                if unique_count <= 2:
                    ...
                    if fit and self.target_column in df.columns:
                        ...
                        df[f"{column}_target_encoded"] = df[column].map(target_means).fillna(target_means.mean())
    ...
    def scale_features(self, df: pd.DataFrame, fit: bool = True) -> pd.DataFrame:
        ...
        binary_columns = [col for col in numerical_columns 
                         if df[col].nunique() <= 2]
    ...
    def _save_pipeline_metadata(self, df: pd.DataFrame, execution_time: float):
        ...
        metadata = {
            ...
            "feature_creation_times": {
                k: v.isoformat() for k, v in self.feature_creation_time.items()
            },
            "selected_features": self.selected_features[:20] if self.selected_features else None,
            "feature_importance": {
                "top_10": self.feature_importance.get("univariate", pd.DataFrame()).head(10).to_dict("records")
                if "univariate" in self.feature_importance else None
            }
        }
        
        # Save to file (in production, save to feature store or database)
        import json
        with open(f"feature_pipeline_metadata_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json", "w") as f:
