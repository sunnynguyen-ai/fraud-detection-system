...
def get_api_health():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return {"status": "unhealthy", "error": f"HTTP {response.status_code}"}, False
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge = {
            ...
            'steps': [
                ...
                {'range': [color_thresholds[0], color_thresholds[1]], 'color': "gray"}
            ],
            'threshold': {
...
def main():
    ...
    if not api_healthy:
        st.error("⚠️ Cannot connect to Fraud Detection API. Please ensure the API is running on http://localhost:8000")
    ...
    if st.button("🎲 Simulate New Transaction") or (auto_refresh and len(st.session_state.transaction_history) < 50):
        # Generate transaction
        ...
        if pred_success:
            ...
            st.error(f"Failed to get prediction: {prediction.get('error', 'Unknown error')}")
    ...
    if st.session_state.transaction_history:
        ...
        with col2:
            ...
            st.metric("Fraud Detected", fraud_detected, delta=f"{fraud_detected/total_transactions:.1%}")
        ...
        if len(df) > 1:
            ...
            fig_ts = make_subplots(
                rows=2, cols=1,
                subplot_titles=['Transaction Amount Over Time', 'Fraud Probability Over Time'],
        ...
        recent_df = df.tail(10)[['transaction_id', 'Amount', 'fraud_probability', 'risk_level', 'is_fraud', 'timestamp']]
        ...
        st.info("No transaction data available. Click 'Simulate New Transaction' to start monitoring.")
