...
class BaseDetector:
    ...
    def prepare_data(self, X: pd.DataFrame, y: pd.Series, 
        ...
        if balance_data:
            ...
            print(f"  ⚖️ Balanced training data: {pd.Series(y_train_balanced).value_counts().to_dict()}")
    ...
    def train(self, X: pd.DataFrame, y: pd.Series, 
        ...
        if tune_hyperparameters:
            ...
            grid_search = GridSearchCV(
                ...
                param_grid, cv=cv_folds, scoring='f1', n_jobs=-1
            )
            grid_search.fit(X, y)
        ...
        print(f"  ✅ Training completed! CV F1: {cv_scores.mean():.4f} ± {cv_scores.std():.4f}")
...
class EnsembleFraudDetector:
    ...
    def __init__(self, weights: Dict[str, float] = None, random_state: int = 42):
        ...
        self.models = {
            ...
            'logistic_regression': LogisticRegressionDetector(random_state=random_state)
        }
    ...
    def train(self, X: pd.DataFrame, y: pd.Series, 
        ...
        try:
            self.explainer = shap.Explainer(best_model, X_train.sample(min(100, len(X_train))))
    ...
    def explain_prediction(self, X: pd.DataFrame, index: int = 0) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, (feature, shap_val) in enumerate(zip(self.feature_names, shap_values.values[0])):
                feature_contributions.append({
                    'feature': feature,
                    'value': sample[feature].iloc[0],
                    'shap_value': float(shap_val),
                    'contribution': 'increases fraud risk' if shap_val > 0 else 'decreases fraud risk'
                })
            ...
            return {
                ...
                'risk_level': 'HIGH' if prediction > 0.8 else 'MEDIUM' if prediction > 0.3 else 'LOW',
...
def test_fraud_detection_models():
    ...
    X_train, X_test, y_train, y_test = rf_detector.prepare_data(X, y, balance_data=False)
